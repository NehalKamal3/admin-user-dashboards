...
$select =" select * from skills where skill_id !=$_GET[id]  ";
 ...
 // move_uploaded_file($img_tmp, '../assets/img/icons/'. $img_name);
   ...
   if($img_size > 2097152) {
     $errors[] = 'plz upload file less than 2 miga';
        ...
        move_uploaded_file($img_tmp, '../front/assets/img/icons/'. $img_name);


       $up="update skills set skill_name ='$name', skill_img='$img_name' where skill_id=$_GET[id]";
       $exct= mysqli_query($conn , $up);
       var_dump($up);

      if($exct ){
       
        $_SESSION['success']='skill updated succesfully';
...
$sel =" select * from skills where skill_id=$_GET[id]  ";
...
?>
  ...
  <div class="mb-1 w-35">
    ...
    <input type="text" class="form-control" name="name"value="<?=$row['skill_name']?>"   >
  
  </div>
  <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label">skill image</label><br>
    <th> <img src="../front/assets/img/icons/<?=$row['skill_img'] ?? 'user.jpg'?>"  style="width:40px; border-radius:50%;"/>

    <input type="file" name="image" class="form-control" value=""  id="exampleInputPassword1">
