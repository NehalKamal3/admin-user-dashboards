...
if(isset($_POST['serv'])){
  if(!empty( $_POST['name'] && $_POST['disc']  )){
    ...
    $disc=validate($_POST['disc']);
      ...
      if(!in_array($img_ext, $allowed_ext)) {
      ...
      }           move_uploaded_file($img_tmp, 'assets/img/serv/'. $img_name);
       ...
       if($img_size > 2097152) {
         $errors[] = 'plz upload file less than 2 miga';
          ...
          if(empty($errors) && isset($_FILES['image']['name'])   ){
           move_uploaded_file($img_tmp, 'assets/img/serv/'. $img_name);
             ...
             $stat = "insert into services (user_id ,ser_name, ser_disc , ser_img) values
              ($_SESSION[id],'$name' ,'$disc' , '$img_name')";
        ...
        $ins =  "insert into services (user_id ,ser_name, ser_disc ) values
        ($_SESSION[id],'$name' ,'$disc' )";
        $query2 = mysqli_query($conn, $ins);
...
?>
        ...
        <input type="text" placeholder="" name="disc" >
        ...
        <input name="image" type="file" id="img">
            ...
            <input class="btn" type="submit" name="serv" value="add">


      
        <div class="social">
            <div class="go"><i class="fab fa-google"></i><a href="index.php"> back</a> </div>
        </div>
    </form>
